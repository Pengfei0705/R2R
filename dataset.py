...
def Patch2Im(Y, win, img_size, stride=1):
    ...
    for i in range(win):
        for j in range(win):
            img[:, i:endw - win + i + 1:stride, j:endh - win + j + 1:stride] = img[:, i:endw - win + i + 1:stride,
                                                                               j:endh - win + j + 1:stride] + Y[:, k,
                                                                                                              :].reshape(
                endc, tempw, temph)
            weight[:, i:endw - win + i + 1:stride, j:endh - win + j + 1:stride] = weight[:, i:endw - win + i + 1:stride,
                                                                                  j:endh - win + j + 1:stride] + 1.
            k = k + 1
    img = img / (weight + 1e-6)
    return img
            ...
            for n in range(patches.shape[3]):
                ...
                for m in range(aug_times - 1):
                    ...
                    h5f.create_dataset(str(train_num) + "_aug_%d" % (m + 1),
                                       data=np.stack((data_aug, data_noisy_aug), axis=0))
